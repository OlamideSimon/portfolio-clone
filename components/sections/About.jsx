        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1, transitiondelay: 0.3, transition: { ease: 'linear', delay: 0.3, duration: 0.7 } }} id="about" className="px-5 lg:px-20 space-y-10 md:space-y-0">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center md:px-10">
                <div className="lg:px-20">
                    <div className="relative rounded-3xl border-dashed border-4 border-purple-200 p-10 md:p-14">
                        <Image
                            src={profile}
                            alt=""
                            className="rounded-3xl"
                        />
                        <div className="absolute bg-blue-900 text-center bottom-2 right-5 md:bottom-10 md:right-10 p-5 rounded-tr-3xl rounded-bl-3xl">
                            <div className="my-0 mx-auto overflow-hidden max-w-[130px]">
                                <div className="whitespace-nowrap font-corpsansmedium transition-all duration-1000" style={{ transform: `translate3d(${-index * 100}%, 0, 0)` }}>
                                    {hobbies.map(({ Icon, hobbie }, index) => (
                                        <div key={index} className='inline-block h-[40px] w-full'>
                                            <Icon size={50} className='grid place-content-center w-full text-white' />
                                            <p className="capitalize text-xl">{hobbie}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
...
const styles = {
    skill: 'm-2 w-fit px-3 py-1 cursor-pointer text-white font-semibold hover:scale-110 transition-all duration-100 rounded-tl-xl rounded-br-xl bg-[#252b3d] hover:bg-purple uppercase'
}